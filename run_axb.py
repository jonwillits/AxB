...
initial perplexity for type should be:
...
NUM_EVAL_STEPS = 10
TRAIN_VERBOSE = False
CAT_EVAL_VERBOSE = False
TYPE_EVAL_VERBOSE = False
...
max_cat_pp = -np.log(1 / master_vocab.master_vocab_size) * train_corpus.num_sequences / num_windows
...
max_num_types = np.max([NUM_AB_TYPES, NUM_X_TEST_TYPES])
max_type_pp = -np.log(1 / max_num_types) * train_corpus.num_sequences / num_windows
print(max_type_pp)
...
for epoch in range(srn.epochs):
    ...
    # train
    srn.train_epoch(train_seqs, TRAIN_VERBOSE, NUM_EVAL_STEPS)
